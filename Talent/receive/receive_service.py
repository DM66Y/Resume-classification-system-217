...
def uplaod_file():
    ...
    if request.method == 'POST':
        ...
        file_dir = os.path.join(os.path.abspath('.'), UPLOAD_FOLDER)
        ...
        file_path= file_dir + str(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time()))) + str(np.random.randint(0, 9999)) + filename
        file.save(file_path)
        ...
        if(file_path.split('.')[-1] == 'docx'):
            ...
            url = 'http://127.0.0.1:4019/api/resume_classify'
